...
class AddWebhook extends PureComponent {
  ...
  render() {
    ...
    const pageContent = (
      <span>
        <p>Webhook，也就是人们常说的钩子,是一个很有用的工具，通过定制 Webhook 来监测你在 PaaS云平台上的各种事件，最常见的莫过于 Build 事件。<br/>
        如果设置了一个监测 Build 事件的 Webhook，那么每当你的这个项目有了构建操作，这个 Webhook 都会被触发，这时 PaaS云平台就会发送一个 HTTP POST 请求到你配置好的地址。
        </p>
      </span>
    );
